...
public class Contract {
    ...
    void addContract(ActionEvent event) {
        ...
        int Customer_ID = getPrimaryKey("tbl_customer", "Customer_ID", "customer_name", tenantName);
        int ContractType_ID = getPrimaryKey("tbl_contract_type", "ContractType_ID", "Contract_type", contractType);

        isContractExists(Customer_ID, roomID, startDate, endDate);
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/db_im_finals", "root", "");
            ...
            statement.setInt(3, ContractType_ID);
    ...
    private boolean isContractExists(int tenant, String room, String startDate, String endDate) {
        String query = "SELECT COUNT(*) FROM tbl_contract WHERE Customer_ID = ? AND Room_ID = ? AND StartDate = ? AND EndDate = ?";
