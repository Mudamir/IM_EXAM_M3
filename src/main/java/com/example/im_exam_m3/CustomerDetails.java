...
public class CustomerDetails implements Initializable {
    ...
    void Addcustomer(ActionEvent event) {
        ...
        if (contact.length() <= 11 || contact.length() <= 13 || !contact.matches("\\d+")) {
        ...
        if (isNameExists(name) || isContactExists(contact)) {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        // Set up TableView with filtered data
        table_tenant.setItems(filteredData);
    }
    ...
    private void populateTableView() {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/db_im_finals", "root", "");
             PreparedStatement statement = connection.prepareStatement("SELECT c.Customer_Name, c.Customer_Contact, ci.City, s.Status, c.Address " +
                     "FROM tbl_customer c " +
            ...
            while (resultSet.next()) {
                ...
                String contact = resultSet.getString("Customer_Contact");
                String city = resultSet.getString("City");
                String employment = resultSet.getString("Status");
                String address = resultSet.getString("Address");
                data.add(new CustomerDetails_Data(name, contact, city, employment, address));
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    @FXML
    void UpdateCustomer_details(ActionEvent event) {
        ...
        if (selectedCustomer != null) {
            ...
            // Get the new contact from the text field
            String newContact = tb_contact.getText();

            // Get the new address from the text field
            ...
            // Validate the contact field to ensure it contains between 11 and 13 digits
            if (newContact.length() < 11 || newContact.length() > 13 || !newContact.matches("\\d+")) {
                // Display an error message indicating that the contact field must contain between 11 and 13 digits
                showAlert(AlertType.ERROR, "Error", "Contact must contain between 11 and 13 digits.");
                return; // Exit the method if contact field is not valid
            }
            ...
            // Update the database with the new name, contact, and address of the selected customer
            updateDatabase( newName, newContact, newAddress);
    ...
    private void updateDatabase(String newName, String newContact, String newAddress) {
        ...
        CustomerDetails_Data selectedCustomer = table_tenant.getSelectionModel().getSelectedItem();
        if (selectedCustomer == null) {
            // No customer selected, exit the method
            return;
        }

        int customerId = selectedCustomer.getCustomerId(); // Assuming you have a method to get the customer ID

        // Construct the SQL query to update the selected customer's details
        String sql = "UPDATE tbl_customer SET ";
        boolean needComma = false;
        if (newName != null && !newName.isEmpty()) {
            sql += "Customer_Name = '" + newName + "'";
            needComma = true;
        }
        if (newContact != null && !newContact.isEmpty()) {
            ...
            sql += "Customer_Contact = '" + newContact + "'";
            needComma = true;
        }
